...
goTo = function (a) {
	...
	return $j(document.body).keyup(e => {
		Array(74).concat([a.filter(function () {
			// console.log($j(this).position())
			return this.getBoundingClientRect().top >> 0 > 0
		}).get(0), a.filter(function () {
			return this.getBoundingClientRect().top >> 0 < 0
		}).get(-1)])[e.keyCode].scrollIntoView()
	});
}
...
traceConsole = (all = 0) => ($j(document.body).children("pre:has(code#code)").length ? trace("#code", all) : $j(document.body).prepend($j('<pre><code class="xml" id="code">&nbsp;</code></pre>').css({
...
<xsl:output method="xml"/>`).replace(/(\<\/xsl\:stylesheet\>)?$/, "</xsl:stylesheet>"));
	z.importStylesheet(y);
	return z.transformToDocument(x).firstChild;
}
dp = new DOMParser();
...
String.prototype.match2 = function (b, ...d) {
	c = [];
	if (!this.length) return c;
	b = b && (b instanceof RegExp && (b.global ? b : new RegExp(b.source, "g")) || typeof b == "string" && new RegExp(b, "g")) || /^.*$/g;
	while (a = b.exec(this)) c.push(a);
...
arr2tree = l => (l.unshift(null, new Node(l.shift())), l.reduce((i, j, k) => k >> 1 && (i[k >> 1][
	["left", "right", "left"][k % 2]
] = j, i.concat(i[k >> 1][
	["left", "right"][k % 2]
])) || i.concat(j), [])[1]);
...
for ([i, j] of Object.entries({
		...
		"AddWeeks": function (d) {
			return this.AddDays(d * 7)
		},
		"AddMonths": function (d) {
			return new Date(this.setMonth(this.getMonth() + d))
		},
		"AddYears": function (d) {
